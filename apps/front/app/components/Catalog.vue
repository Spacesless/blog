<template>
  <div
    ...
    class="relative"
    :class="{
      'w-0 pt-4': !isShowCatalog,
      'border-l border-dashed border-[var(--border-color)] lt-xl:border-l-0':
        isShowCatalog,
    }"
  >
    <div
      v-show="isShowCatalog"
      class="sticky top-0 w-50 lt-xl:static lt-xl:w-full"
    >
      <p
        class="relative h-10 px-4 mb-2 leading-10 text-[var(--color-secondary)] border-b border-dashed border-[var(--border-color)]"
      >
    ...
    <span
      v-show="!isShowCatalog"
      class="sticky top-4 inline-block w-5 p-1 text-xs text-white cursor-pointer rounded-r-md bg-gradient-to-br from-[#24c6dc] via-[var(--color-primary)] to-[#5433ff]"
      @click="toggleShow"
      >文章目录</span
    >
  </div>
