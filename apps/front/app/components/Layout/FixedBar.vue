  ...
  <div class="fixed right-2 bottom-4 z-998 lt-md:hidden">
    ...
    >
      <div
        ...
        :class="live2dShow ? 'text-[var(--color-primary)]' : 'text-[var(--color-secondary)]'"
        @click="toggleWaifu"
      >
        <Icon name="ph:smiley" />
      </div>
    ...
    >
      <div
        class="w-10 h-10 mt-2.5 text-xl leading-10 text-center cursor-pointer select-none rounded-full bg-[var(--bg-normal)] shadow-[var(--shadow-3-right)] hover:text-[var(--color-primary)]"
        :class="particleActive ? 'text-[var(--color-primary)]' : 'text-[var(--color-secondary)]'"
        @click="toggleBubble"
      >
    ...
    >
      <transition name="fade-transform">
        <div
          ...
          @click="backTop"
        >
          <span class="group-hover:hidden">{{ scrollPercent }}%</span>
          <Icon name="ph:arrow-up" class="hidden group-hover:inline-block" />
        </div>
      </transition>
    </el-tooltip>
  </div>
