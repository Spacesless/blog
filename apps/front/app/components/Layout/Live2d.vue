<template>
  <div v-if="isShow" ref="waifuRef" class="group fixed right-0 bottom-0 z-997 w-52 h-55 transition-[margin-top] duration-300 lt-lg:hidden hover:cursor-grab" @mousedown="dragMove">
    <transition name="fade-transform" mode="out-in">
      <div v-show="tipsShow" class="absolute top-0 w-[185px] h-15 px-2.5 py-1.5 overflow-hidden text-xs text-[#303133] text-ellipsis bg-white/80 border border-[var(--color-primary)] rounded-xl shadow-[0_1px_3px_#66ccff]" v-html="tips" />
    </transition>
    <canvas
      id="js-live2d"
      width="208"
...
const texturesId = computed(() => Number(appStore.configs?.live2d_texture) || 1)
...
const clickTips = ['萝莉控是什么呀', '(๑•́ ∀ •̀๑)', '๑乛◡乛๑嘿嘿', '！⌇●﹏●⌇', '(ó﹏ò｡)']
  ...
  if (val) {
    if (!isLoaded.value) {
      ...
      if (import.meta.client) {
        // @ts-expect-error - vendor library
        await import('/vendor/live2d/index.js').catch(() => {})
      }
    }
    handleShowLive2d()
  } else {
...
function loadModel(mId: number, tId = 1) {
  if (import.meta.client && (window as any).loadlive2d) {
...
function handleTakePhoto() {
  ...
  if (import.meta.client && (window as any).Live2D) {
    (window as any).Live2D.captureName = 'Pio.png'
    ;(window as any).Live2D.captureFrame = true
  }
}
...
function dragMove(e: MouseEvent) {
  ...
  document.onmousemove = (e) => {
    ...
    if (left < 0) left = 0
    else if (left > clientWidth - el.clientWidth) left = clientWidth - el.clientWidth
    if (top < 0) top = 0
    else if (top > clientHeight - el.clientHeight) top = clientHeight - el.clientHeight
