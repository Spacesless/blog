  ...
  >
    <div>
      ...
      <div class="max-w-1200px mx-auto mt-[var(--grid-space)] px-4">
        ...
        <a class="inline-block px-4 py-1.5 mx-2 text-xs text-[var(--color-primary)] cursor-pointer bg-[var(--menu-active)] rounded-2xl" @click="handleSearchHot('web')">web</a>
        <a class="inline-block px-4 py-1.5 mx-2 text-xs text-[var(--color-primary)] cursor-pointer bg-[var(--menu-active)] rounded-2xl" @click="handleSearchHot('Api')">Api</a>
        <a class="inline-block px-4 py-1.5 mx-2 text-xs text-[var(--color-primary)] cursor-pointer bg-[var(--menu-active)] rounded-2xl" @click="handleSearchHot('徒步')">徒步</a>
      </div>
      ...
      <div v-loading="fetchLoading" class="max-w-1200px mx-auto my-4 px-4">
        ...
        <div>
          <ul v-for="(item, index) in searchList" :key="index">
            <li class="relative min-h-12 pl-22 pr-4 py-3 mb-0.5 bg-[var(--bg)] rounded-[var(--border-radius)]">
              <span class="absolute top-1/2 left-0 w-22 -mt-6 text-4xl italic text-center">{{ index + 1 + (listQuery.page - 1) * 10 }}</span>
              <NuxtLink class="text-lg text-[var(--color-heading)] hover:text-[var(--color-primary)]" :to="item.url" @click="onClose">
              ...
              <span v-if="item.categoryUrl" class="text-sm">
                ...
                <NuxtLink class="text-[var(--color-secondary)]" :to="item.categoryUrl" @click="onClose">
                  {{ item.categoryName }}
                </NuxtLink>
              </span>
              <p class="pt-1.5 text-sm leading-relaxed text-[var(--color-text)]" v-html="highlightKeyword(item.content)" />
            </li>
          </ul>
        </div>
...
const resultInfo = ref<{ keyword: string; classify: string }>({ keyword: '', classify: '' })
const total = ref(0)
...
async function fetchList() {
  ...
  try {
    ...
    const findClassify = classifyOptions.find(item => item.value === listQuery.value.classify)
    resultInfo.value = {
      keyword: listQuery.value.keyword,
...
function highlightKeyword(str: string) {
  ...
  return (str || '').replace(reg, (arg: string) => `<span class="text-red-500">${arg}</span>`)
}
</script>
