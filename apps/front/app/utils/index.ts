...
export function convertToTree<
  T extends { id: number; parent_id: number | null; children?: T[] },
>(list: T[]): T[] {
...
export function detailUrl(
  type: "article" | "bangumi",
  item: { id: number; pathname?: string | null },
): string {
...
/**
 * 解析列表页路由参数
 * 支持新格式 ['slug'] / ['slug', '2']，兼容旧格式 ['id-page']（如 ['4-2']、['list-2']）
 */
export function parseListRoute(segments: string[]): {
  ...
  if (segments.length === 1 && /-\d+$/.test(segments[0])) {
    const lastDashIndex = segments[0].lastIndexOf("-");
    const idPart = segments[0].slice(0, lastDashIndex);
    const pagePart = segments[0].slice(lastDashIndex + 1);
    return {
      id: idPart === "list" ? null : idPart,
      page: Number(pagePart) || 1,
    };
  }

  // 新格式：/type/slug 或 /type/slug/2
  const id = segments[0] === "list" ? null : segments[0];
  const page = segments[1] ? Number(segments[1]) || 1 : 1;
  return { id, page };
}
...
export function buildListPath(
  ...
  const base = id ?? "list";
  return page > 1 ? `/${type}/${base}/${page}` : `/${type}/${base}`;
}

/**
 * 构建栏目列表入口 URL，优先使用 filename(slug)
 */
export function categoryListUrl(
  category?: { id: number; type: string; filename?: string | null } | null,
): string {
  if (!category) return "";
  return `/${category.type}/${category.filename || category.id}`;
}
